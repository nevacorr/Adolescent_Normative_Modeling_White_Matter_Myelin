def evaluate_splines_cv(X_train,y_train, roi_ids, agemin,agemax,cv_dir,spline_orders=[1, 2],spline_knots_list=[2, 3, 4],n_folds=5):
    import os
    import shutil
    import numpy as np
    import pandas as pd
        ...
        for order in spline_orders:
            for knots in spline_knots_list:
                ...
                for train_idx, val_idx in kf.split(X_train, strat_feature):  # stratify by sex for example
                    ...
                    fold_dir = os.path.join(cv_dir, f"{roi}_o{order}_k{knots}_f{fold_idx}")
                    makenewdir(fold_dir)
                    ...
                    create_design_matrix('train', agemin, agemax, order, knots, [roi], fold_dir)
                    create_design_matrix('test', agemin, agemax, order, knots, [roi], fold_dir)
                    ...
                    try:
                        ...
                        metrics['roi'] = roi
                        metrics['spline_order'] = order
                        metrics['spline_knots'] = knots
                        metrics['fold'] = fold_idx + 1

                        allfoldsdf = pd.concat([allfoldsdf, pd.DataFrame([metrics])], ignore_index=True)
    ...
    # Compute mean metrics across folds per ROI & spline config
    mean_perf_metrics = allfoldsdf.groupby(['roi', 'spline_order', 'spline_knots'], as_index=False).mean()
